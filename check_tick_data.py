...
async def check_tick_data():
    try:
        collection = get_collection('tick_data')
        
        # Check total count
        count = await collection.count_documents({})
        print(f"Total tick_data records: {count}")
        ...
        if count > 0:
            ...
            if latest:
                ...
                print(f"Latest record price: {latest_record.get('price')}")
                
                # Check time difference
                if latest_record.get('timestamp'):
                    latest_time = latest_record['timestamp']
                    if isinstance(latest_time, str):
                        latest_time = datetime.fromisoformat(latest_time.replace('Z', '+00:30'))
        ...
        for symbol in symbols:
            ...
            if symbol_count > 0:
                ...
                if latest_symbol:
                    ...
                    latest_price = latest_symbol[0].get('price')
                    print(f"  Latest {symbol} time: {latest_time}, price: {latest_price}")
        
    except Exception as e:
        print(f"Error: {e}")

if __name__ == "__main__":
    asyncio.run(check_tick_data()) 